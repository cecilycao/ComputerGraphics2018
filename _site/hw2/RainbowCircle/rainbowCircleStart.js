...
function initializeScene(){
  ...
  for (var j = 0; j <= 2 * Math.PI / 360 * 360; j += theta){
    ...
    for (var i = 0; i < number_of_steps; i++){
      ...
      var down2 = new THREE.Vector3(Math.cos((j + theta)) * (r + i * step), 
        Math.sin(j + theta) * (r + i * step), 0);
      var up1 = new THREE.Vector3(Math.cos(j) * (r + (i + 1) * step), 
        Math.sin(j) * (r + (i + 1) * step), 0);
      var up2 = new THREE.Vector3(Math.cos(j+theta) * (r + (i + 1) * step), 
        Math.sin(j+theta) * (r + (i + 1) * step), 0);
